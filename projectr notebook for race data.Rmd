...
#Boxplot of whole number of votes per county by race
boxplot(race2[c(5:10)])
...
barplot(WIWhitesBoxplotSubset$White, col = rainbow(20))
...
barplot(CAWhitesBoxplotSubset$White, col = rainbow(20))
...
WIBlacksBoxplotSubset = subset(race2, race.sa=="WI", select = c(race.area_name, race.Population, Black, race.sa))

barplot(WIBlacksBoxplotSubset$Black, col = rainbow(20))

#California Black Voting whole number values 
CABlacksBoxplotSubset = subset(race2, race.sa=="CA", select = c(race.area_name, race.Population, Black, race.sa))

barplot(CABlacksBoxplotSubset$Black, col = rainbow(20))
...
race2 =data.frame(race$fips, race$area_name, race$sa,  race$Population, bpop, wpop, napop, apop, pipop, mupop, race$sa, race$area_name)

totBl = sum(race2$Black)
totWh = sum(race2$White)
totNa = sum(race2$Native.American)
totAs = sum(race2$Asian)
totPi = sum(race2$Pacific.Islander)
totMu = sum(race2$X2..Races)


#Comparison of total number of voters by race
totals = c(totAs, totBl, totMu, totNa, totPi, totWh)
barplot(totals, col=rainbow(6))

#legend.text = c("Asian", "Black", "Multiracial", "Native American", "Pacific Islander", "White")
#, legend.text = c("A", "B", "M", "NA", "PI", "W"),  args.legend = list(x = "bottomright")
```
