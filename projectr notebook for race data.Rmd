...
# if (!(pres16results['cand'] == 'Donald Trump' && pres16results['cand'] == 'Hillary Clinton')) {
#   pres16results['cand'] <- 'other'
# }
...
race = county_facts[c(1:4, 11:17)]
setnames(race, old=c("PST045214", "state_abbreviation", "RHI125214","RHI225214", "RHI325214", "RHI425214", "RHI525214", "RHI625214", "RHI725214"), new=c("Population", "sa", "White", "Black", "Native American", "Asian", "Pacific Islander", "2+ Races", "Latino"))
...
race2 =data.frame(race$fips, race$area_name, race$sa,  race$Population, bpop, wpop, napop, apop, pipop, mupop, lpop)

#issue: State fips code and totals are mixed with counties, so sums don't give real total votes

#Total number of votes from each race for the enitre election
totBl = (sum(race2$Black)-race2$Black[1])/1000000
totWh = (sum(race2$White)-race2$White[1])/1000000
totNa = (sum(race2$Native.American)-race2$Native.American)[1]/1000000
totAs = (sum(race2$Asian)-race2$Asian[1])/1000000
totPi = (sum(race2$Pacific.Islander)-race2$Pacific.Islander[1])/1000000
totMu = (sum(race2$X2..Races)-race2$X2..Races[1])/1000000
totLa = (sum(race2$Latino)-race2$Latino[1])/1000000

totals = c(totAs, totBl, totMu, totNa, totPi, totWh, totLa)
```

```{r}

#barplot of total number of voters by race
barplot(totals, col=rainbow(7), names.arg = c("Asian", "Black", "Mixed", "Native", "Pacific", "White", "Latino"), main = "Total Votes by race", xlab = 'Races', ylab = "Votes in Millions")

#Same barplot, but withput the white vote for a better view of the others
barplot(totals[-c(6,7)], col=rainbow(7), names.arg = c("Asian", "Black", "Mixed", "Native", "Pacific"), main = "Total Votes by race, Excluding Whites & Latinos", xlab = 'Races', ylab = "Votes in Millions")
```
...
scatterplot(CAWhitesBoxplotSubset$White, CAWhitesBoxplotSubset$race.Population, main="California White Vote Versus Population")
...
usmapDvR=map('county', fill= TRUE, col = classcolors[mpe16rwvd9$lead]) 
...
map('county', "Colorado", fill= TRUE, col = classcolors[mpe16rwvd9$lead])map
map('county', "illinois", fill= TRUE, col = classcolors[mpe16rwvd9$lead])
...
BvW =data.frame(nbpop, nwpop, county_facts["state_abbreviation"])

sBvW=subset(BvW, state_abbreviation=="WI", select = c(Black, White, state_abbreviation))
...
trump=subset(mpe16rwvd9, Candidates=="Donald.Trump", select = c(county, total_votes, Candidates, Votes))
hillary=subset(mpe16rwvd9, Candidates=="Hillary.Clinton", select = c(county, total_votes, Candidates, Votes))
